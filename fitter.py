...
def fitter(wavelength_in,data_in,error_in,models_in,options):
	...
	probs = statistics.convert_chis_to_probs(chis,len(wavelength))
	prob_nonzero = np.where(probs>0.01)
	...
	if np.size(prob_nonzero)< 10:
		print "Not enough good solutions for errors! Mix solutions instead!"
		extra_fit_list 		= mix(np.asarray(final_fit_list)[best_fits[:bf]].tolist(),\
								np.asarray(final_fit_list)[:num_models].tolist()+np.asarray(final_fit_list)[prob_nonzero].tolist(),\
								np.min(chis))
	else:
		extra_fit_list = []

	# extra_fit_list 		= mix(np.asarray(final_fit_list)[best_fits[:bf]].tolist(),final_fit_list,\
	# 						np.min(chis))
