...
def get_SFD_dust(long,lat,dustmap='ebv',interpolate=True):
    ...
    * 'mask'
    ...
    if dml == 'ebv' or dml == 'eb-v' or dml == 'e(b-v)' :
        dustmapfn='dustmaps/SFD_dust_4096_%s.fits'
    elif dml == 'i100':
        dustmapfn='dustmaps/SFD_i100_4096_%s.fits'
    elif dml == 'x':
        dustmapfn='dustmaps/SFD_xmap_%s.fits'
    elif dml == 't':
        dustmapfn='dustmaps/SFD_temp_%s.fits'
    elif dml == 'mask':
        dustmapfn='dustmaps/SFD_mask_4096_%s.fits'
    else:
        dustmapfn=dustmap
    ...
    if '%s' not in dustmapfn:
        ...
        if polename=='ngp':
            ...
            n=[-1]
            if sum(b < 0) > 0:
                b=b#print 'using sgp file when lat > 0 present... put %s wherever "ngp" or "sgp" should go in filename'
        else:
...
def hpf(flux,windowsize=0,w_start=0):
    ...
    #     windowsize = 0
    if w == 0 and windowsize == 0:
        #print "HPF parameters not specified: choosing default window (stepsize=num flux points / 100)"
        w = int(D/100.0)
        windowsize = 0.0
...
def determine_attenuation(wave,data_flux,error_flux,model_flux,parameters,age,metal):
